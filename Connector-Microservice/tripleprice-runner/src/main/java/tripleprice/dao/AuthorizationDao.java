...
*/
	...
	@Query("SELECT new tripleprice.dto.AuthorizationDto(a.sysMenu.url as patternDispatcherUrl, b.roleName as roleName) from AuthorizationModel a left join a.sysRoles b where a.sysMenu.url is not null order by a.sysMenu.url ")
	...
	@Query(value = "select new  tripleprice.dto.AuthorizationDto( SA.createdTime as createTime, SA.modifiedTime as modifyTime, SA.id as id, SA.sysMenu.menusName as menuName,  SA.parent.id as parentId,  SA.isUpdate as isUpdate,  SA.isDelete as isDelete,  SA.isInsert as isInsert,  SA.isRead as isRead ) from AuthorizationModel SA  where SA.id =:nidMenu ")
